...
class AddEventScreen extends StatefulWidget {
  const AddEventScreen({super.key, required this.editor, this.existing});
...
class _AddEventScreenState extends State<AddEventScreen> {
  ...
  void initState() {
    ...
    _type = e?.type ?? EventType.enableArea;
    _targetId = e?.targetId;
  ...
  bool get _targetIsArea => [
        EventType.enableArea,
        EventType.disableArea,
        EventType.toggleArea,
      ].contains(_type);

  bool get _targetIsConn => [
        EventType.enableConnection,
        EventType.disableConnection,
        EventType.toggleConnection,
      ].contains(_type);

  bool get _targetIsState => [
        EventType.activateGameState,
        EventType.deactivateGameState,
        EventType.toggleGameState,
      ].contains(_type);
  ...
  Widget build(BuildContext context) {
    ...
    final conns = widget.editor.connections.values.toList()..sort((a, b) => a.id.compareTo(b.id));
    final flags = widget.editor.gamestates.values.toList()..sort((a, b) => a.id.compareTo(b.id));
    final isNew = widget.existing == null;

    return Scaffold(
      appBar: AppBar(title: Text(isNew ? 'Novo Evento' : 'Editar Evento')),
      body: Align(
          ...
          children: [
            ...
            if (_targetIsArea)
              DropdownButtonFormField<int>(
                value: areas.any((a) => a.id == _targetId) ? _targetId : null,
                decoration: const InputDecoration(labelText: 'Area alvo'),
                items: areas.map((a) => DropdownMenuItem(value: a.id, child: Text(a.name))).toList(),
                onChanged: (v) => setState(() => _targetId = v),
              )
            else if (_targetIsConn)
              DropdownButtonFormField<int>(
                value: conns.any((c) => c.id == _targetId) ? _targetId : null,
                decoration: const InputDecoration(labelText: 'Conexao alvo'),
                items: conns.map((c) {
                  final a = widget.editor.areas[c.areaA]?.name ?? '${c.areaA}';
                  final b = widget.editor.areas[c.areaB]?.name ?? '${c.areaB}';
                  return DropdownMenuItem(value: c.id, child: Text('$a ↔ $b'));
                }).toList(),
                onChanged: (v) => setState(() => _targetId = v),
              )
            else if (_targetIsState)
              DropdownButtonFormField<int>(
                value: flags.any((f) => f.id == _targetId) ? _targetId : null,
                decoration: const InputDecoration(labelText: 'Gamestate alvo'),
                items: flags.map((f) => DropdownMenuItem(value: f.id, child: Text(f.name))).toList(),
                onChanged: (v) => setState(() => _targetId = v),
              ),
            const SizedBox(height: 16),
            SwitchListTile(
